...
bool manDefendedFromFire(Point manPosition,vector<Point> firePositions,int arrY,int arrX,int** maze, int fireNum){
    for(int i = 0; i < fireNum;i++){
        if(BFS(maze,manPosition,firePositions[i],arrY,arrX) != -1) return 0;
    }
    return 1;
}
